...
BOOL ST_USBDevice::GetLog(HANDLE hDev, char *Buf, ULONG Size)
{
  BOOL bRet;
  DWORD dwRet;

  bRet = DeviceIoControl(hDev, IOCTL_USBCAM_GET_LOG, 0, 0, Buf, Size, &dwRet, 0);

  if (bRet && (dwRet != 0)) return TRUE;
  return FALSE;
}
...
//---------------------------------------------------------------------------
// Прочитать регистр сопротивления по I2C
BOOL KCL001::GetLight(HANDLE hDev, PBYTE pData, BYTE num)
{
	if (num > 4) return false;
	PWORD pDat = (PWORD)pScan;
	if (UploadI2C(hDev, 0, 1, pDat, ((0x2C00>>8)+num)<<8))
	{
		*pData = 255 - *pDat;
		return true;
...
//---------------------------------------------------------------------------
// Записать регистр сопротивления по I2C
BOOL KCL001::SetLight(HANDLE hDev, BYTE Data, BYTE num)
{
	if (num > 4) return false;
	PWORD pData = (PWORD)pScan;
	pData[0] = 255 - Data;
	if (DownloadI2C(hDev, 0, 1, pData, ((0x2C00>>8)+num)<<8)) return true;
	return false;
}
...
//---------------------------------------------------------------------------
// Прочитать АЦП
BOOL KCL001::GetADC(HANDLE hDev,BYTE page_num, BYTE Reg, PBYTE pData)
{
 // WORD Value;
  VENDOR_OR_CLASS_REQUEST_CONTROL Req;
...
// Работа с двигателем.
// cmd - номер команды
// dir -  1 - считать состояние порта двигателя, 0 - записать значение команды:

BOOL KCL001::MotorCmd(HANDLE hDev, BYTE cmd, WORD value)
{
  VENDOR_OR_CLASS_REQUEST_CONTROL Req;
  ...
  return VendorRequest(hDev, &Req, 4);
}
//---------------------------------------------------------------------------
// Прочитать текущее количество шагов двигателя
BOOL KCL001::GetMotorState(HANDLE hDev, PDWORD pState)
{
	VENDOR_OR_CLASS_REQUEST_CONTROL Req;
...
//---------------------------------------------------------------------------
// Прочитать текущее количество шагов двигателя
BOOL KCL001::GetStep(HANDLE hDev, PSHORT pStep)
{
  VENDOR_OR_CLASS_REQUEST_CONTROL Req;
...
//---------------------------------------------------------------------------
// Получить текущую позицию указанной линейки в шагах двигателя
BOOL KCL001::GetCurPos(HANDLE hDev, PSHORT pCurPos)
{
  VENDOR_OR_CLASS_REQUEST_CONTROL Req;
...
//---------------------------------------------------------------------------
// Прочитать конечники
BOOL KCL001::GetTreilers(HANDLE hDev, bool* pTrail)
{
  VENDOR_OR_CLASS_REQUEST_CONTROL Req;
  ...
  if(VendorRequest(hDev, &Req, 2)) {
    pTrail[0] = ((bool*)pMem)[0];
    pTrail[1] = ((bool*)pMem)[1];
    return TRUE;
  }
  return FALSE;
}
...
BOOL KCL001::GoRead(HANDLE hDev, WORD Len)
{
  VENDOR_OR_CLASS_REQUEST_CONTROL Req;
  ...
  return VendorRequest(hDev, &Req, 0);
}
//---------------------------------------------------------------------------
// Запустить автоматическое считывание линейки
BOOL KCL001::StartAutoRead(HANDLE hDev, WORD Len)
{
  VENDOR_OR_CLASS_REQUEST_CONTROL Req;
...
BOOL KCL001::StopAutoRead(HANDLE hDev)
{
  VENDOR_OR_CLASS_REQUEST_CONTROL Req;
...
//---------------------------------------------------------------------------
// Остановить автоматическое считывание линейки
BOOL KCL001::Switch_mode(HANDLE hDev, int mode)
{
  VENDOR_OR_CLASS_REQUEST_CONTROL Req;
...
//---------------------------------------------------------------------------
// Запись регистра выбранного АЦП
BOOL KCL001::SetADC(HANDLE hDev, BYTE page_num, BYTE Reg, BYTE Data)
{
  ...
  if (VendorRequest(hDev, &Req, 1))
  {
	  ...
	  Req.requestType = 2; // 1=class, 2=vendor
	  Req.recepient = 0;
	  return VendorRequest(hDev, &Req, 1);
  } else return false;

}
//---------------------------------------------------------------------------
// Прочитать поток в указанную область памяти
int KCL001::ReadScanPipe(HANDLE hDev, PWORD pMemScan, DWORD Len)
...
//******************************************************************
// Прочитать размеры окна камеры по I2C
BOOL KCL001::GetFrameProperty(HANDLE hDev, PFRAME_PROPERTY pFrame)
{
	PWORD pData = (PWORD)pScan;
	BYTE temp = 0;
	if ( (UploadI2C(hDev, 0x1, 1, &pData[0], 0x5D02))&(UploadI2C(hDev, 0x2, 1, &pData[1], 0x5D02))&
...
//---------------------------------------------------------------------------
// Записать размеры окна камеры по I2C
BOOL KCL001::SetFrameProperty(HANDLE hDev, PFRAME_PROPERTY pFrame)
{
	PWORD pData = (PWORD)pScan;
	memset(pData, 0, 8);
	pData[0] = pFrame->top + 0x0C;
...
//---------------------------------------------------------------------------
// Установить смещение камеры
BOOL KCL001::SetOffset(HANDLE hDev, short *offset)
{
	//WORD temp = 0x499;
	//DownloadI2C(hDev, 0x62, 1, &temp, 0x5D02);
	PWORD pValue = (PWORD)pScan;
	memset (pValue, 0, 4*sizeof(WORD));
...
//---------------------------------------------------------------------------
// Прочитать смещение камеры
BOOL KCL001::GetOffset(HANDLE hDev, short *pValue)
{
	WORD temp = 0x499;
	DownloadI2C(hDev, 0x62, 1, &temp, 0x5D02);
	PWORD pOffset = (PWORD)pScan;
	memset (pOffset, 0, 4*sizeof(WORD));
...
//---------------------------------------------------------------------------
// Установить усиление камеры
BOOL KCL001::SetGain(HANDLE hDev, BYTE Value)
{
	PWORD pData = (PWORD)pScan;
	pData[0] = (0x7F)&Value;
  if (DownloadI2C(hDev, 0x35, 1, pData, 0x5D02)) return true;
...
//---------------------------------------------------------------------------
// Прочитать усиление камеры
BOOL KCL001::GetGain(HANDLE hDev, PBYTE pValue)
{
	PWORD pData = (PWORD)pScan;
	if (UploadI2C(hDev, 0x35, 1, pData, 0x5D02))
	{
...
//---------------------------------------------------------------------------
// Получить изображение с устройства
BOOL KCL001::GetImage(HANDLE hDev, PBYTE image)
{
  int Size;
  DWORD	Len = current_width*current_height;
  Size = ReadPipeMem(hDev, image, Len);
  if(Size == (int)Len) return TRUE;
  return FALSE;
}


int KCL001::ReadPipeMem(HANDLE hDev, PBYTE pMemCam, DWORD Len)
{
  BOOL bRet = false;
  DWORD dwRet = 0;
  BULK_TRANSFER_CONTROL TransfControl;
